...
[role="lead"]
Créez une relation de réplication pour un système de fichiers FSx for ONTAP dans NetApp Workload Factory afin d'éviter une perte de données en cas de sinistre imprévu. La réplication est prise en charge entre deux systèmes de fichiers FSx for ONTAP, ainsi qu'entre Cloud Volumes ONTAP ou un système ONTAP sur site et un système de fichiers FSx for ONTAP.

.Description de la tâche
La réplication offre une protection contre la perte de données si la région où résident vos données subit un sinistre.
