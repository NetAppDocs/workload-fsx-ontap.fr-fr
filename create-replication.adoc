...
[role="lead"]
Créez une relation de réplication pour un système de fichiers FSx for ONTAP dans NetApp Workload Factory afin d’éviter une perte de données en cas de sinistre imprévu. Vous pouvez répliquer des données entre deux systèmes de fichiers FSx for ONTAP, ou entre un système ONTAP local et un système de fichiers FSx for ONTAP.

Pour la migration de machines virtuelles de stockage, effectuez l'opération de basculement immédiatement après avoir créé une relation de réplication.
...
Après avoir créé une relation de réplication pour un cas d'utilisation de migration, vous devez effectuer un basculement de la réplication afin de finaliser la migration des données et des paramètres de configuration de la machine virtuelle de stockage vers le système de fichiers FSx for ONTAP. Le basculement de la réplication migre définitivement les données et les paramètres de configuration de la machine virtuelle de stockage du système de fichiers source vers le système de fichiers FSx for ONTAP. Lors du basculement, les données sont répliquées une dernière fois. La machine virtuelle de stockage source est mise hors ligne au début du basculement.
...
* link:initialize-replication.html["Initialiser la réplication"] pour une nouvelle relation de réplication.
