...
summary: 'Découvrez et répliquez les données ONTAP locales dans NetApp Workload Factory afin qu"elles puissent être utilisées comme sauvegarde dans le cloud.' 
---
= Utiliser les données du cluster ONTAP sur site dans NetApp Workload Factory
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
Découvrez et répliquez les données ONTAP locales dans NetApp Workload Factory afin qu'elles puissent être utilisées comme sauvegarde dans le cloud.
...
Découvrez un cluster ONTAP sur site dans NetApp Workload Factory afin de pouvoir répliquer les données vers un système de fichiers Amazon FSx for NetApp ONTAP .

.Avant de commencer
Assurez-vous d'avoir les éléments suivants avant de commencer :

* Un système de fichiers FSX pour ONTAP pour la réplication.
* Un lien actif pour associer le cluster local détecté. Si vous n'avez pas de lien, vous devrez link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["créez-en un"^].
...
+
.. *Lien* : Sélectionnez un lien.  Le lien sera associé au cluster sur site pour créer une connectivité entre le cluster et Workload Factory.
+
...
.. *Identifiants ONTAP* : Fournissez les éléments suivants :
+
*** GovCloud accounts : Saisissez l’ARN avec les informations d’identification ONTAP.
*** Tous les autres comptes : saisissez les identifiants ONTAP pour le cluster ONTAP sur site. Assurez-vous que l’utilisateur dispose des autorisations requises.
...
+
*** *Auto* : la stratégie de hiérarchisation par défaut lors de la création d'un volume à l'aide de l'interface utilisateur Workload Factory FSx pour ONTAP .  Classez toutes les données froides qui incluent les données utilisateur et les instantanés dans le niveau de stockage du pool de capacité pour une période spécifique.
...
== Supprimer un cluster ONTAP sur site de NetApp Workload Factory

Supprimez un cluster ONTAP sur site de NetApp Workload Factory si nécessaire.
...
. Sélectionnez le menu Actions et sélectionnez *Supprimer de Workload Factory*.


.Résultat
Le cluster ONTAP sur site est supprimé de NetApp Workload Factory.
